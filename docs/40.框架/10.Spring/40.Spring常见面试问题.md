...
## Spring如何创建一个 Bean 对象的？

通过 `new UserService()` 创建的对象和 Spring 容器帮我们创建的 Bean 对象是不一样的，少了依赖注入这一步。需要给加了 @Autowire 或 @Resource 注解的对象注入到对象中。

也就是说，Spring 在创建 Bean 对象的时候，会先去看看这个对象内部需要哪些对象，先去创建内部依赖的对象，依赖的对象创建完毕后再创建该对象，并把依赖的对象注入进去。
