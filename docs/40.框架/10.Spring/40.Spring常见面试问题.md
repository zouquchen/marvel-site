...
存放可以生成 Bean 的工厂。假如 A 类实现了FactoryBean，那么依赖注入的时候不是 A 类，而是 A 类产生的Bean。
...
Bean 的创建流程：实例化生成普通对象，放入单例池 — 属性赋值 — 初始化前 — 初始化 — 初始化后（AOP）生成代理对象 ，放到单例池

假设只使用二级缓存，伪代码如下：

```
A a = new A();  // 实例化A，发现A依赖B

B b = new B();  // 那么实例化B，B也依赖A
b.a = a;        // 将刚刚实例化好的a注入到b中，此时b已经创建完成

a.b = b;        // 将b注入到a中
a.x = x;        // 继续进行属性赋值...
```

以上的过程没什么问题，但是如果我们考虑到初始化后（AOP）生成代理对象，如果生成代理对象，那么 b 注入的就是普通的对象， 而不是代理对象，也就是 b.a 拿错了对象。实际上，A 创建好的对象是一个代理对象 a，但是 b 中注入的却是普通对象 a，这样 b 就依赖错对象了。所以需要使用三级缓存，把 a 的代理对象注入到 b 当中。

为了解决这个问题需要提前进行 AOP。比如，创建 A 时发现需要 B，去单例池中找 B 是否已经被创建，发现 B 没有创建。那么下面将要创建 B，首先，将 A 放到一个“对象正在创建”的池中，创建对象 B，发现需要 A，此时发现“对象正在创建”的池中含有 A，那么就出现了循环依赖。此时，也就是在创建 B 的过程中，需要对 A 对象提前进行 AOP，生成 A 的代理对象，并将 A 的代理对象注入到 B 对象中。

需要注意的是，如果 A 还循环依赖 C，C 再创建的时候就不要再对 A 提前进行 AOP 了，他应该直接获取到 A 的代理对象。
