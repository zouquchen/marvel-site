...
`refresh()` 是 AbstractApplicationContext 内的模板方法，这 12 种方法的默认实现在 AbstractApplicationContext 内，同时可以被 GenericApplicationContext 和 AnnotationConfigApplicationContext重写。不过看了一下，这12个方法都没有被子类重写。

通过通过注解文件启动容器，可以使用 ClassPathXmlApplicationContext 进行创建。对于刚刚的12个方法，有一些被 GenericWebApplicationContex 实现。
